...
(async () => {
    ...
    function frame() {
        ...
        if (ImGui.TreeNode("lithium's atm")) {
            ...
            ImGui.Image(new ImTextureRef(atmTexId), new ImVec2(atm.width / 1.3, img.height / 1.3));

            ImGui.TreePop();
        }
        if (ImGui.TreeNode("ratware")) {
            ImGui.Text("very dead project, executor that was last updated in march (worse than awp)");
            ImGui.Image(new ImTextureRef(ratTexId), new ImVec2(ratware.width / 1.3, img.height / 1.3));
